...
def play(game, card):
    ...
    if card.position != -1:
        for obs in game:
            if card.player == obs.player and card.position == obs.position and obs.isVisible == False:
                obs.isVisible = True
                obs.isPlayable = True
...
def print_game(game, mode = 'B'):
    ...
    if mode == 'omni':
        ...
        for pos in range(5):
            string = '   '
            for c in game:
                if c.player == 'A' and c.inHand == False and c.isVisible == True and c.isPlayable == True and c.position == pos:
                    string = c.name()
            print(string, end = ' ')
        print("\nINVI ", end = '')
        for pos in range(5):
        ...
        for pos in range(5):
            string = '   '
            for c in game:
                if c.player == 'B' and c.inHand == False and c.isVisible == True and c.position == pos and c.isPlayable == True:
                    string = c.name()
            print(string, end = ' ')
        print("\nINVI ", end = '')
        for pos in range(5):
